...
export default function Login() {
  ...
  async function login() {
    ...
    try {
      ...
      setLoginResponse(`Error logging in`);
    }
  }
      ...
      <form onSubmit={handleSubmit}>
        <h1 className={styles.title}>
          ...
          {loginInResponse === "Invalid login credentials" && (
            <div className={styles.signUpSuccessful}>{loginInResponse}</div>
          )}
          {loginInResponse === "Error logging in" && (
            <div className={styles.signUpUnSuccessful}>{loginInResponse}</div>
          )}
        </h1>
        <input
