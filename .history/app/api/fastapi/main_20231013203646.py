...
def fake_save_user(userAddress: str, email: str, password: str, firstName: str, lastName: str):

    hashed_password = pwd_context.hash(password)
    user = User(userAddress=userAddress, email=email, hashed_password=hashed_password,
    ...
    sql = "INSERT INTO users (userAddress, email, password, firstName, lastName) VALUES (%s, %s, %s, %s, %s)"
    val = (userAddress, email, hashed_password, firstName, lastName)
    # Prepare SQL query
...
async def signup(user: Data):
    user_saved = fake_save_user(user.userAddress,
...
async def login(user: UserIn):
    ...
    user_from_db = User(userAddress=userDetails[0]["userAddress"], email=userDetails[0]["email"],
...
@app.post("/addpayment/")
async def add_payment(item: dict):
    ...
    sql = "INSERT INTO creditcards (CreditCardNumber, ExpiryDate, CVC, BillingAddress, CardName, UserEmail) VALUES (%s, %s, %s, %s, %s, %s)"
    val = (item['CreditCardNumber'], item['ExpiryDate'],
           item["CVC"], item["BillingAddress"], item["CardName"], item["UserEmail"])
    cursor.execute(sql, val)
...
@app.post("/hascreditcard/")
async def get_credit_card(email: EmailSchema):

    connection = mysql.connector.connect(**db_config)
    cursor = connection.cursor(dictionary=True)

    query = f"SELECT * FROM creditcards WHERE UserEmail = '{email.email}'"
    cursor.execute(query)
    ...
    return result[0]  # return the first credit card if there are multiple
...
@app.get("/checkcardexists/")
async def check_card(card_number: cardNumber):
    return card_number.cardNumber
    cnx = mysql.connector.connect(**db_config)
    cursor = cnx.cursor()

    query = ("SELECT * FROM creditcards WHERE CreditCardNumber = %s")
    cursor.execute(query, (card_number.cardNumber,))
    ...
    return {"message": "Card exists in the database"}


w3 = Web3(Web3.HTTPProvider("HTTP://127.0.0.1:7545"))
chain_id = 1337
my_address = "0xE3fee827fe22613B65ffaA193b61A27eD7DD614e"
private_key = "0xe61319c77dc1daca4a7adf696010172d35b97c4bd4eac592d8e4aa822a4e7cae"
...
@app.post("/purchase/")
async def make_purchase(data: purcahseInfo):

    try:
        tx_hash = purch_contract.functions.makePurchase(
            data.itemName, data.itemPrice, data.firstName, data.lastName, data.email).transact({'from': data.userAddress})
