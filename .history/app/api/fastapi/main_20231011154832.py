...
const item = {
      itemName: formData.cardNumber,
      itemPrice: formData.expiryDate + "-01",
      userAddress: formData.CVC,
      firstName: formData.userEmail,
      lastName: formData.fullName,
      email: formData.email,
    };
@app.post("/purchase/")
async def make_purchase(purchase: dict):
    try:
        tx_hash = purch_contract.functions.makePurchase(
            purchase["itemName"], purchase["itemPrice"], purchase["firstName"], purchase["lastName"], purchase["email"]email).transact({'from': userAddress})
