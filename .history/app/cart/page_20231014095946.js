...
export default function Cart() {
    ...
    for (let i = 0; i < cartItems.length; i++) {
      for (let j = 0; j < cartItems[i].quantity; j++) {
        ...
        let data = {
          ...
          userAddress: user.userAddress, // replace with actual user address
  ...
  async function checkCreditCardExists() {
    axios
      .get(`http://127.0.0.1:8000/check_card/${formData.cardNumber}`)
      .then((response) => {
        return true;
      })
      .catch((error) => {
  ...
  function purchase(event) {
    ...
    if (checkCreditCardExists()) {
      ...
      axios
        .post("http://127.0.0.1:8000/addpayment/", payment)
